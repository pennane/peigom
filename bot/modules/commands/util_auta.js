...
const { YOUTUBE_API_KEY } = require('../utilities/config')
...
function getCommandInfo(path) {
  ...
  files.forEach((file) => {
    ...
    if (!YOUTUBE_API_KEY && command.type.some((type) => type === 'music')) {
      return
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    function createBaseEmbed() {
      const embed = new Discord.MessageEmbed()
        ...
        .setTitle(`${CLIENT_CONFIG.APP.NAME}  \`${configuration.name}\``)
    ...
    function defaultResponseEmbed() {
      ...
      embed.addField(
        ...
        `\`${prefix}${configuration.name} ${CLIENT_CONFIG.APP.NAME} \``,
        false
      )
      embed.addField(
    ...
    function commandsForTypeEmbed(type) {
      ...
      foundCommands.forEach((command) => {
        embed.addField(
          ` ${command.type.indexOf('admin') !== -1 || command.type.indexOf('superadmin') !== -1 ? ':unlock: ' : ''}${
            command.name
          }`,
    ...
    function commandEmbed(commandname) {
      ...
      embed
        .setDescription(`Tietoa komennosta: \`${prefix}${command.name}\` (${[...command.type].join(' ')})`)
    ...
    function botInfoEmbed() {
      ...
      embed
        ...
        .addField(`:vertical_traffic_light: Versio:`, `${CLIENT_CONFIG.APP.VERSION}`)
        .addField(
          `:question: Mikä ihmeen ${CLIENT_CONFIG.APP.NAME} ?`,
          `${CLIENT_CONFIG.APP.NAME} on tälläne [node.js](https://nodejs.org/) discord botti, joka pistää röttöilijät kuriin.`
        )
    ...
    function messageEmbed(message) {
      let embed = createBaseEmbed()
      embed.setTitle('Botin kommentti:').setDescription(message)
      return embed
    }
    ...
    } else if (args[1].toLowerCase() === CLIENT_CONFIG.APP.NAME.toLowerCase()) {
