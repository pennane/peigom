...
let COMMAND_SPAM_PROTECTION = CLIENT_CONFIG.COMMAND_SPAM_PROTECTION
...
module.exports.parseMsg = function (msg, client) {
  ...
  if (!hasPrefix && hasBadWords) {
    msg.react(client.emojis.cache.get('304687480471289866')).catch((err) => {
      logger.log(3, err)
    })
  }
  ...
  if (
    disabled.hasOwnProperty(channelId) &&
    disabled[channelId] === 'disabled' &&
    command.name !== 'bottitoimiitäällä'
  ) {
    msg.channel
      .send('Botti ei toimi tällä tekstikanavalla.')
      .then((msg) => {
