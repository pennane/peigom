...
module.exports.log = (activityId, content) => {
    return new Promise((resolve, reject) => {
        ...
        if (!CLIENT_CONFIG.get('LOG_USED_COMMANDS') && activityId === 1) return;
        ...
        */
        ...
        let activities = {
            1: {
                ...
                messageGenerator: (content) => `\r\n[Command] ${hh}.${m}.${ss} [User: ${content.msg.author.username}:${content.msg.author.id}] [Command: ${content.command}] ${content.args[1] ? '[Args: ' + content.args.filter((arg, i) => i !== 0) + ']' : null} @${content.msg.channel.guild.name}:${content.msg.channel.guild.id}#${content.msg.channel.name}`
            },
            2: {
            ...
            7: {
                ...
                finally: (data) => { console.info(chalk.gray("|-- New member on " + member.guild.name + ":" + member.guild.id + ", " + member.user.username + ":" + member.user.id)) }
            },
            8: {
            ...
            9: {
                ...
                finally: (data) => { console.info(chalk.gray(`|-- New guild joined: ${guild.name}:${guild.id}. This guild has ${guild.memberCount} members.`)) }
            },
            10: {
                name: "Guild Removed from the Client",
                messageGenerator: (content) => `\r\n[Removed server] Server: ${content.name}:${content.id} When: ${todayPrecise}`,
                finally: (data) => { console.info(chalk.gray(`|-- Bot removed from: ${guild.name}:${guild.id}`)) }
            },
            11: {
                name: "Command Failed to Load",
                messageGenerator: (content) => `\r\n[Failed command] Command '${content}' failed to load`,
                finally: (data) => { console.info(chalk.red(`|-- Command ${chalk.yellow(content)} failed to load. Check ${chalk.white("./log/")} for more`)) }
            },
            12: {
                name: "Invalid Command",
                messageGenerator: (content) => `\r\n[Invalid command] Failed command: ${content.name}. Reason: ${content.reason}`,
                finally: (data) => { console.info(chalk.red(`|-- Command ${chalk.yellow(content.name)} failed to construct itself. Check ${chalk.white("./log/")} for more`)) }
            },
            13: {
                name: "Command Failed to load with an Error Stack",
                messageGenerator: (content) => `\r\n[Faulty command file] '${content.file}' failed to load. \r\n ${content.err}`,
                finally: (data) => { console.info(chalk.red(`|-- File ${chalk.yellow(content.file)} failed to load. Check ${chalk.white("./log/")} for more`)) }
            },
            14: {
