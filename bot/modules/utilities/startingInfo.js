...
module.exports.set = (client) => {
  ...
  console.info(
    chalk.yellow('| Loaded: ') +
      ...
      ' users with ' +
      CLIENT_CONFIG.PREFIX +
      'raha'
  )
  console.info(chalk.yellow('| Loaded: ') + badwords.length + ' forbidden words')
  console.info(
    chalk.yellow('| Log user used commands: ') +
      (CLIENT_CONFIG.LOG_USED_COMMANDS ? chalk.green('true') : chalk.red('false'))
  )
  console.info(
    chalk.yellow('| Command spam protection: ') +
      (CLIENT_CONFIG.COMMAND_SPAM_PROTECTION.STATE ? chalk.green('true') : chalk.red('false'))
  )
}
