...
const shortDate = (): string => {
  const { hour, minute } = getDateParts()
  return `${hour}:${minute}`
}

const longDate = (): string => {
  const { day, month, hour, minute } = getDateParts()
