...
const cache: Map<string, UserHistory> = new Map()

const SpamProtection = (user: Discord.User): AllowedState | DeniedState => {
  const AMOUNT_OF_COMMANDS = COMMAND_SPAM_PROTECTION.AMOUNT_OF_COMMANDS
  const AMOUNT_OF_SECONDS = COMMAND_SPAM_PROTECTION.AMOUNT_OF_SECONDS

  const now = Date.now()

  const userHistory = cache.get(user.id)
  ...
  if (!userHistory.timestamps[AMOUNT_OF_COMMANDS - 1]) {
    ...
    now - userHistory.timestamps[AMOUNT_OF_COMMANDS - 1] >
    ...
    state = {
      allowed: false,
      remaining: 10000 - (now - userHistory.timestamps[2])
    }
  }
