...
let adminUsers = JSON.parse(fs.readFileSync('./data/adminUsers/data.json', 'utf8'))
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!args[2]) {
      text +=
        ...
        ' <anna/poista> <0-' +
        (guildRoles.length - 1) +
        '>`, ja kyseisen roolin bot-oikeudet päivittyvät.'
    ...
    fs.writeFile('./data/adminUsers/data.json', JSON.stringify(adminUsers), function (err) {
      if (err) {
