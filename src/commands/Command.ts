...
class Command {
  ...
  constructor(initializer: CommandInitializer) {
    ...
    this._requireGuild =
      typeof configuration.requireGuild === 'boolean'
        ? configuration.requireGuild
        : true
  ...
  static isMemberAdminAuthorized(message: Discord.Message): boolean {
    ...
    if (
      AppConfiguration.DISCORD.SUPER_ADMIN_AUTHORIZED.includes(
        message.author.id
      )
    )
      return true
  ...
  static isMemberSuperAdminAuthorized(message: Discord.Message): boolean {
    return AppConfiguration.DISCORD.SUPER_ADMIN_AUTHORIZED.includes(
      message.author.id
    )
  }
