...
const reservedNames = ['utility', 'komennot', 'fun', 'sound', 'music', 'image', 'admin', 'other']
    ...
    command.triggers.forEach((trigger) => {
      ...
      loadedTriggers.forEach((triggers, command) => {
        if (triggers.indexOf(trigger) !== -1) {
          throw new Error(
            `Warning! Command '${command.name}' interferes with command '${command}' with the trigger '${trigger}'`
          )
        }
      })
