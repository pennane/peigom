...
const yt = new YouTube(YOUTUBE_API_KEY)
...
async function play(guild) {
  ...
  let stream = ytdl(track.videoDetails.video_url, { filter: 'audioonly', quality: 'lowest' })
  let dispatcher = serverQueue.connection.play(stream, {
    //  type: 'opus',
    volume: serverQueue.options.volume
  })
...
const queueMethods = {
    ...
    if (!serverQueue) {
      ...
      if (serverQueue.tracks.length > 1) {
        embed.addField(
          ...
          `${[...serverQueue.tracks]
            .splice(1)
            .map((t, i) => `\`${i + 1}\`: [${t.title}](${t.video_url})`)
            .join(`\n`)}`
        )
      }
  ...
  nowPlaying: function (args) {
    ...
    if (serverQueue.tracks.length > 0 && serverQueue.connection && serverQueue.connection.speaking) {
      ...
      let ytTime = msToReadable(serverQueue.tracks[0].videoDetails.lengthSeconds * 1000)
      embed
  ...
  remove: function (args) {
    ...
    if (
      serverQueue &&
      serverQueue.tracks &&
      serverQueue.tracks.length > 1 &&
      toRemove &&
      parseInt(toRemove) > 0 &&
      serverQueue.tracks[parseInt(toRemove)]
    ) {
  ...
  volume: function (args) {
    ...
    if (serverQueue && serverQueue.dispatcher) {
      ...
      if (computedVolume >= 0) {
        ...
        msg.channel.send('Volyymiks asetettu ' + computedVolume)
        return
      } else {
        msg.channel.send('Tän hetkinen volyymi on ' + serverQueue.options.volume)
        return
